...
export default function UserDashboardPage() {
  ...
  const curData =
    thisTxs.length > 0
      ? thisTxs
      : recentMonths[1]
      ? transactions.filter((t) => t.date.startsWith(recentMonths[1].month))
      : transactions.slice(0, 20);
  const prevData =
    lastTxs.length > 0
      ? lastTxs
      : recentMonths[0]
      ? transactions.filter((t) => t.date.startsWith(recentMonths[0].month))
      : [];
  ...
  return (
    <div className="space-y-8 max-w-screen mx-8 my-3">
      <div className="rounded-2xl bg-linear-to-r from-violet-600/20 via-purple-600/10 to-transparent border border-violet-500/20 p-6 flex items-center justify-between">
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        <div className="lg:col-span-3 rounded-2xl surface border border-(--card-border) p-5">
          ...
          <div className="h-56 min-h-52 w-full">
